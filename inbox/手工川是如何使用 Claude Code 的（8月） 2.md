...
本文尽可能系统总结手工川过去一年 Vibe Coding 的相关经验，重点厘清大家在使用 Claude Code 中的一些细节问题。
...
![基于 cc 研发的 cc 面板，see: https://github.com/MarkShawn2020/claude-code-manager](https://poketto.oss-cn-hangzhou.aliyuncs.com/f8a3862dc9fb6434d5b2516383a3dffc.png?x-oss-process=image/quality,q_90/rotate,0)
...
我们积累了不少优化 prompt （包括 CLAUDE.md）的手段，限于篇幅无法一一展开，此处仅分享一下我们可扩展版本的用户级 CLAUDE.md 文件，希望对你有所帮助。
...
但值得注意的是，git worktree 并发很爽，合并是个小小的头疼问题，尽管吴恩达教程里选择了让cc自行合并，但如果你是个 indie vibe coder（独立Vibe开发者），你绝不会喜欢这种被block的感觉。

所以，对于inide vibe coder，我的建议是：建议并行开发多个项目，每个项目内使用串行开发多个features。
...
![](https://poketto.oss-cn-hangzhou.aliyuncs.com/203f65ddad696e5e5daf273ba2a04fd8.png?x-oss-process=image/quality,q_90/rotate,0)
...
- claude code 是个软件，内部有一套自动拆解并解决问题的机制
...
### gpt5、gemini、kimi 等到底哪个更好

## 磨刀不误砍柴工

### ~/.claude/commands/ （仅显示最常用的几个）
...
我的很多场景，mcp 都不是最优解，比如有人会用 Playwright 去自动截图，但我感觉效率太慢了，我会倾向于使用 `code-inspector-plugin`直接定位到源码。（我们还二开了一个更适合大模型的版本）

也用了好几次据说非常好用的 context7，但实测效果都不怎么好，最后我都是自己维护外部知识库。
...
├── electron
├── engineering
├── executive-highlights-tldr-of-the-article-1755382099165.md
├── github
├── google
├── html-refs
├── json
...
├── vercel
├── wechat
└── xinference-docker-image-1755595011092.md

28 directories, 2 files
```

而且 context7 也不能解决更通用的网页数据问题，这种场景一般用 brave-search、jina、Playwright等，但我还是有自己更高效的解决方案，用我们内部的 lovpen-sider……
