...
上面代码指定项目的入口脚本为`./src/index.tsx`，它的格式为 ES6 模块。如果没有`type`字段，`index.tsx`就会被解释为 CommonJS 模块。
...
// 实际加载的是 ./node_modules/es-module-package/src/index.tsx
```
...
// esm.mjs
import { createRequire } from '@/content/zh/coding/es-6/module.md';

const require = createRequire(import.settings.url);
...
```javascript
import cjsModule from '../index.tsx';
